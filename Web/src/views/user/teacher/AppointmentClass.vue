        ...
        <a-modal v-model:open="modelInfoVisible" @ok="updateEvent">
            <div class="form-model">
                <a-input
                    ...
                    @update:value="newValue => infoEvent.title = newValue"
                    id="title" placeholder="Add title" class="space"
                    :readonly="!infoEvent.isAuthor"
                />
                <a-time-picker
                ...
                >
                    <template #addonAfter>
                        <a-select :value="infoEvent.typeTime" @update:value="newValue => infoEvent.typeTime = newValue"
                                  style="width: 100px;" :disabled="!infoEvent.isAuthor"
                        >
                            <a-select-option value="Minutes">Minutes</a-select-option>
                            <a-select-option value="Hours">Hours</a-select-option>
                            <a-select-option value="Days">Days</a-select-option>
            ...
            <template #footer>
                <a-button key="delete" v-if="infoEvent.isAuthor" @click="deleteEvent" type="primary" danger>Delete</a-button>
...
const getListAppointment = async () => {
    try {
        const [appointmentRes, guestAppointmentRes] = await Promise.all([
            appointmentApi.getListAppointment(),
            appointmentApi.getListAppointmentOfGuest()
        ])

        const appointments = appointmentRes.data.map(event => ({
            title: event.title,
        ...
        const guestAppointments = guestAppointmentRes.data.map(event => ({
            title: event.title,
            start: new Date(event.start),
            end: new Date(event.end),
            location: event.location,
        ...
        events.value = [...appointments, ...guestAppointments];
    } catch (err) {
...
const openModelInfo = (event) => {
    ...
    infoEvent.value.isAuthor = event.isAuthor
}
